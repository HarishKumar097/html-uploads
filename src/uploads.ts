...
type EventNameType = "chunkAttempt" | "chunkSuccess" | "error" | "progress" | "success" | "online" | "offline" | "chunkAttemptFailure";

interface UserProps {
    endpoint: string,
    file: File,
    retryChunkAttempt?: string | number,
    delayRetry?: string | number | undefined,
...
let defaultChunkSize: number = 16384;
...
export class Uploader {
    ...
    lastProgressBeforeInterupption: number;
    currentVideoUploadProgress: number;
    ...
    constructor(props: UserProps) {
        ...
        this.lastProgressBeforeInterupption = 0;
        this.currentVideoUploadProgress = 0;
        ...
        if (typeof window !== "undefined") {
            window.addEventListener("online", () => {
                ...
                if (this.isOffline && this.failedChunkRetries < this.retryChunkAttempt && !this.isAborted && this.totalChunks > 0 ) {
                    this.isOffline = false;
            ...
            window.addEventListener("offline", () => {
                this.isOffline = true;
                this.uploadOffset = this.currentVideoUploadProgress;

                if (this.totalChunks !== this.successiveChunkCount && !this.isUploadCompleted && !this.isAborted && this.failedChunkRetries < this.retryChunkAttempt && this.totalChunks > 0 ) {
                    if (this.activeXhr) {
    ...
    abort() {
        if (this.activeXhr && this.totalChunks > 0 ) {
            this.activeXhr?.abort();
            this.activeXhr = undefined;
            this.isAborted = true;
            this.retryUpload();
    ...
    pause() {
        ...
        if (!this.isOffline && !this.isPaused && !this.isAborted && this.failedChunkRetries < this.retryChunkAttempt && this.totalChunks > 0) {
            this.isPaused = true;
            if (this.activeXhr) {
                this.activeXhr?.abort();
                this.activeXhr = undefined;
    ...
    resume() {
        ...
        if (this.isPaused && !this.isOffline && !this.isAborted && this.failedChunkRetries < this.retryChunkAttempt && this.totalChunks > 0 ) {
            this.isPaused = false;
    ...
    chunkUploadFailureHandler = async (res: { statusCode: number, responseBody: any, url: string, method: string }) => {

        if (!this.isOffline && !this.isPaused && !this.isAborted && this.totalChunks > 0) {
    ...
    submitHttpRequest(options: { method: "PUT"; url: string, body: Blob | File }) {
        ...
        return new Promise((resolve) => {
            ...
            xhr.upload.onprogress = (event) => {
                ...
                const progressChunkSize = this.streamFile.size - this.nextChunkRangeStart
                const progressPerChunk = (progressChunkSize) / this.streamFile.size / remainingChunks;
                const successfulProgress = this.nextChunkRangeStart / this.streamFile.size;
                const checkTotalChunkSize = event.total ?? this.chunkSize;
                const currentChunkProgress = (event.loaded + this.uploadOffset) / checkTotalChunkSize;
                const chunkProgress = currentChunkProgress * progressPerChunk;
                const overallProgress = Math.min((successfulProgress + chunkProgress) * 100, 100);
                this.emitEvent("progress", overallProgress);
                this.currentVideoUploadProgress = event.loaded;
            };
                ...
                if (xhr.readyState === 4) {
                    ...
                    if ([200, 201, 204, 206].includes(xhr.status)) {
                        ...
                        let prevChunkUploadedInterval =
                            (prevChunkUploadedTime.getTime() - this.prevSegmentStart.getTime()) / 1000;
                        ...
                        this.nextChunkRangeStart = this.nextChunkRangeStart + this.currentChunkSize
                        this.validateUploadStatus()
                    } else {
            ...
            xhr.setRequestHeader("Content-Type", this.streamFile.type);
            xhr.setRequestHeader("Content-Range", `bytes ${this.nextChunkRangeStart}-${this.nextChunkRangeStart + this.currentChunkSize - 1}/${this.streamFile.size}`);
    ...
    requestChunk() {
        ...
        if (this.totalChunks > 0 && !this.isOffline && !this.isPaused && !this.isAborted && navigator.onLine) {
            ...
            if (this.videoChunkProcessor) {
                ...
                if (this.chunkOffset === 0) {
                    ...
                    currentChunk = this.videoChunkProcessor.getChunk(this.nextChunkRangeStart, (this.chunkSize * (this.chunkOffset + 1)));
                }
            }
